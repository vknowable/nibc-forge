...
apt-get update && apt-get install libcurl4-openssl-dev libudev-dev jq busybox -y && apt-get clean

cp /root/bin/namada* /usr/local/bin/
cp /root/bin/cometbft /usr/local/bin/
cd /root
